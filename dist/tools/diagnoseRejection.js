...
export async function diagnoseRejection(args) {
    ...
    const patterns = REJECTION_PATTERNS.map((pattern) => {
        const matchedIndicators = pattern.indicators.filter((indicator) => combinedText.includes(indicator.toLowerCase()));
...
function extractKeyQuotes(comments) {
    ...
    const criticalSentences = sentences.filter((s) => criticalKeywords.some((k) => s.toLowerCase().includes(k)));
    return criticalSentences.slice(0, 5).map((s) => s.trim() + ".");
}
function calculateSeverity(patterns, editorDecision) {
    const avgConfidence = patterns.length > 0
        ? patterns.reduce((sum, p) => sum + p.confidence, 0) / patterns.length
        : 0;
    const hasRejectKeyword = editorDecision?.toLowerCase().includes("reject") ||
...
function generateResponseStrategy(patterns) {
    const strategies = {};
...
function getPatternStrategy(pattern) {
    const strategies = {
        so_what: [
...
//# sourceMappingURL=diagnoseRejection.js.map