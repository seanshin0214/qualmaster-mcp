...
export async function suggestMethodology(args) {
    ...
    const contextInfo = await searchKnowledgeBase(`${research_question} 질적연구 방법론`, "traditions", 2);
    return {
        research_question,
...
function determineQuestionType(question) {
    if (question.includes("무엇") || question.includes("어떤"))
        return "what";
    if (question.includes("어떻게"))
        return "how";
    if (question.includes("왜"))
        return "why";
    if (question.includes("경험"))
        return "experience";
    return "descriptive";
}
function determineFocusArea(question) {
    if (question.includes("경험") || question.includes("느낌"))
        return "experience";
    if (question.includes("과정") || question.includes("변화"))
        return "process";
    if (question.includes("문화") || question.includes("조직"))
        return "culture";
    if (question.includes("이야기") || question.includes("삶"))
        return "story";
    if (question.includes("사례"))
        return "case";
    return "general";
}
function calculatePhenomenologyFit(analysis, purpose, participants) {
    let score = 50;
    const reasons = [];
...
function calculateGroundedTheoryFit(analysis, purpose, participants) {
    let score = 50;
    const reasons = [];
    const considerations = [];
    if (analysis.hasProcessFocus) {
...
function calculateEthnographyFit(analysis, purpose, participants) {
    let score = 50;
    const reasons = [];
    const considerations = [];
    if (analysis.hasCultureFocus) {
...
function calculateNarrativeFit(analysis, purpose, participants) {
    let score = 50;
    const reasons = [];
    const considerations = [];
...
function calculateCaseStudyFit(analysis, purpose, participants) {
    let score = 50;
    const reasons = [];
    const considerations = [];
...
function getNextSteps(methodology) {
    const steps = {
        phenomenology: [
            "연구 현상을 명확히 정의하기",
            "에포케(판단중지) 연습하기",
...
//# sourceMappingURL=suggestMethodology.js.map