...
export async function reviewPaper(args) {
    // Defensive coding
    ...
    // Identify top issues
    const allCriteria = sectionScores.flatMap((s) => s.criteria.map((c) => ({
        ...c,
        section: s.korean,
        gap: c.max - c.earned,
    })));
    ...
    const criteria = [
        {
            ...
            max: 5,
            check: (t) => t.length > 200 && (t.includes("최근") || t.includes("중요") || t.includes("문제")),
        },
        {
            criterion: "Gap - 문헌의 공백 식별",
            max: 5,
            check: (t) => t.includes("그러나") || t.includes("하지만") || t.includes("부족") || t.includes("gap"),
        },
        {
            criterion: "So What - 연구 중요성",
            max: 5,
            check: (t) => t.includes("필요") || t.includes("중요") || t.includes("기여"),
        },
        {
            criterion: "Contribution - 기여 명시",
            max: 5,
            check: (t) => t.includes("기여") || t.includes("contribution") || t.includes("확장"),
        },
    ];
    return {
...
function evaluateLiterature(text) {
    const section = extractSection(text, "literature");
    const criteria = [
        {
            criterion: "Integration - 문헌 통합",
            max: 7,
            check: (t) => t.includes("통합") || t.includes("종합") || t.includes("연결"),
        },
        {
            criterion: "Synthesis - 비판적 종합",
            max: 7,
            check: (t) => t.includes("반면") || t.includes("대조적") || t.includes("비판"),
        },
        {
            criterion: "Critical Analysis - 비판적 분석",
            max: 6,
            check: (t) => t.includes("한계") || t.includes("문제점") || t.includes("부족"),
        },
    ];
    return {
        section: "literature",
...
function evaluateTheory(text) {
    const section = extractSection(text, "theory");
    const criteria = [
        {
            criterion: "Novelty - 새로움",
            max: 10,
            check: (t) => t.includes("새로운") || t.includes("novel") || t.includes("최초"),
        },
        {
            criterion: "Logic - 논리적 전개",
            max: 8,
            check: (t) => t.includes("따라서") || t.includes("그러므로") || t.includes("때문에"),
        },
        {
            criterion: "Mechanisms - 메커니즘 설명",
            max: 7,
            check: (t) => t.includes("메커니즘") || t.includes("mechanism") || t.includes("통해"),
        },
        {
            criterion: "Boundary Conditions - 경계조건",
            max: 5,
            check: (t) => t.includes("조건") || t.includes("경우") || t.includes("상황에서"),
        },
    ];
    return {
        section: "theory",
...
function evaluateDiscussion(text) {
    const section = extractSection(text, "discussion");
    const criteria = [
        {
            criterion: "Implications - 함의",
            max: 8,
            check: (t) => t.includes("함의") || t.includes("implication") || t.includes("시사점"),
        },
        {
            criterion: "Limitations - 한계",
            max: 6,
            check: (t) => t.includes("한계") || t.includes("limitation") || t.includes("제한"),
        },
        {
            criterion: "Future Research - 향후 연구",
            max: 6,
            check: (t) => t.includes("향후") || t.includes("future") || t.includes("추후"),
        },
    ];
    return {
        section: "discussion",
...
function evaluateWritingQuality(text) {
    const criteria = [
        {
            criterion: "Clarity - 명확성",
            max: 4,
            check: (t) => t.length > 1000,
        },
        {
            criterion: "Flow - 논리적 흐름",
            max: 3,
            check: (t) => t.includes("첫째") || t.includes("둘째") || t.includes("마지막"),
        },
        {
            criterion: "Academic Conventions - 학술적 관례",
            max: 3,
            check: (t) => t.includes("연구") || t.includes("분석"),
        },
    ];
    return {
        section: "writing",
...
function getGrade(percentage) {
    if (percentage >= 90)
        return "A (출판 가능)";
    if (percentage >= 80)
        return "B (Minor Revision)";
    if (percentage >= 70)
        return "C (Major Revision)";
    if (percentage >= 60)
        return "D (Reject & Resubmit)";
    return "F (Reject)";
}
function getAMRChecklist() {
    return {
...
function getRevisionPriorities(issues) {
    return issues.slice(0, 3).map((issue, i) => `${i + 1}. [${issue.section}] ${issue.criterion}: ${issue.feedback}`);
}
//# sourceMappingURL=reviewPaper.js.map