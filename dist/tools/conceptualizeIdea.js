...
export async function conceptualizeIdea(args) {
    ...
    const relatedConcepts = await searchKnowledgeBase(`${idea} ${phenomenon || ""} 개념 이론`, "exemplars", 3);
...
function developConcept(idea, phenomenon, existingConcepts) {
...
function generateConceptName(idea) {
    const keywords = idea
        .split(/\s+/)
        .filter((w) => w.length > 3)
        .slice(0, 2);
...
function generateNamingOptions(idea, analysis) {
    const options = [];
...
function defineBoundaryConditions(idea, phenomenon) {
    return [
...
function getRelatedTheories(idea) {
    return [
        "검토할 이론 1: [관련성 높은 이론]",
...
function getIntegrationPossibilities(idea, existingConcepts) {
    return [
...
function getConceptPaperStructure() {
    return {
        introduction: [
            "현상 소개 및 연구 동기",
            "기존 이론/개념의 한계",
...
//# sourceMappingURL=conceptualizeIdea.js.map