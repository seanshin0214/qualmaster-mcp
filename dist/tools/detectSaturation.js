...
export async function detectSaturation(args) {
    // Defensive coding
...
function calculateSaturationMetrics(codes, newCodesBySource) {
    const sources = Object.keys(newCodesBySource);
    const totalSources = sources.length;
    // Calculate cumulative new codes
    let cumulativeTotal = 0;
    const cumulativeNewCodes = [];
    ...
    const recentNewCodes = recentSources.reduce((sum, s) => sum + (newCodesBySource[s]?.length || 0), 0);
...
function assessSaturation(metrics, level) {
    const evidence = [];
    ...
    else if (metrics.new_code_rate <= threshold.maxRate * 2 &&
    ...
    if (metrics.total_sources < threshold.minSources) {
        evidence.push(`데이터 소스 수(${metrics.total_sources})가 권장 최소치(${threshold.minSources}) 미만`);
    }
...
function generateVisualizationData(newCodesBySource) {
    const sources = Object.keys(newCodesBySource);
    const newCodesBySourceArray = [];
...
function analyzeTrend(newCodes) {
...
function getSaturationGuidelines(level) {
    const guidelines = {
        code: {
...
//# sourceMappingURL=detectSaturation.js.map