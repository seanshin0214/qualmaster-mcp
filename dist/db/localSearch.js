 ...
 * 검색 점수 계산 (키워드 매칭)
 */
function calculateScore(doc, queryTerms) {
 ...
 */
function extractContext(content, queryTerms, maxLength = 500) {
 ...
 * 지식베이스 검색
 */
export async function searchLocal(query, category = "all", nResults = 5) {
    ...
    if (category !== "all") {
        docsToSearch = knowledgeCache.filter((doc) => doc.category === category ||
