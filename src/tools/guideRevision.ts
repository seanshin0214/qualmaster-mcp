...
interface CommentAnalysis {
  type: "critique" | "suggestion" | "question" | "praise" | "concern";
  typeKorean: string;
  tone: string;
  requiresMajorChange: boolean;
  keyConcerns: string[];
}

function analyzeComment(comment: string): CommentAnalysis {
  const lowerComment = comment.toLowerCase();

  // Determine type
  let type: CommentAnalysis["type"] = "critique";
  let typeKorean = "비판";
...
function getRevisionStrategies(
  pattern: string | undefined,
  analysis: CommentAnalysis
): RevisionStrategy[] {
  const allStrategies: RevisionStrategy[] = [
    {
...
function getWhenToUse(strategy: string): string {
...
function getPatternSpecificAdvice(pattern: string): Record<string, string[]> {
...
function getRevisionChecklist(strategies: RevisionStrategy[]): string[] {
  const baseChecklist = [
  ...
  const strategyChecklist = strategies.flatMap((s) =>
    s.examples.map((e) => `${s.korean} 전략 적용: ${e}`)
  );

  return [...baseChecklist, ...strategyChecklist.slice(0, 3)];
}

function getDosAndDonts(): Record<string, string[]> {
  return {
    do: [
      "모든 코멘트에 감사 표현으로 시작",
      "구체적인 변경 사항과 위치 명시",
...
function getExampleResponses(commentType: CommentAnalysis["type"]): string[] {
