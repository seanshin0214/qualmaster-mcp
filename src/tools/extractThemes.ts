...
function clusterCodes(codes: string[]): Map<string, string[]> {
...
function generateThemeName(codes: string[]): string {
...
function generateThemeDescription(codes: string[], mode: string): string {
  const prefix = mode === "inductive" ? "데이터에서 도출된" : "이론에 기반한";
  return `${prefix} ${codes.length}개의 코드를 포괄하는 주제`;
}

function generateSubthemes(codes: string[]): string[] {
...
function findExemplarQuotes(codes: string[], segments: string[]): string[] {
  const quotes: string[] = [];
...
function identifyRelationships(
  ...
  for (let i = 0; i < themes.length; i++) {
    for (let j = i + 1; j < themes.length; j++) {
      const overlap = themes[i].codes.filter((c) =>
        themes[j].codes.some((c2) => c.includes(c2) || c2.includes(c))
      );
...
function deriveOverarchingTheme(themes: Theme[]): string {
  if (themes.length === 0) return "주제 없음";
  if (themes.length === 1) return themes[0].name;

  return `${themes[0].name}을(를) 중심으로 한 ${themes.length}개 주제의 통합적 이해`;
}

function calculateCoverage(codes: string[], themes: Theme[]): string {
  const themedCodes = new Set(themes.flatMap((t) => t.codes));
  const coverage = (themedCodes.size / codes.length) * 100;
  return `${coverage.toFixed(1)}%`;
}

function findOrphanCodes(codes: string[], themes: Theme[]): string[] {
  const themedCodes = new Set(themes.flatMap((t) => t.codes));
  return codes.filter((c) => !themedCodes.has(c));
}

function generateRecommendations(themes: Theme[], codes: string[]): string[] {
...
function getBraunClarkeChecklist(): Record<string, boolean | string> {
  return {
    "1_데이터에_충실한_코딩": "확인 필요",
    "2_주제_내_일관성": "확인 필요",
    "3_주제_간_구분": "확인 필요",
