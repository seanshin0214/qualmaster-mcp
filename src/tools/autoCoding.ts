...
function generateCodesForSegment(
  segment: string,
  methodology: string,
...
function extractDescriptiveCodes(segment: string): string[] {
  const codes: string[] = [];
...
function generateInterpretiveCodes(segment: string, methodology: string): string[] {
  const codes: string[] = [];
...
function hasRelatedConcept(segment: string, lens: string): boolean {
...
function getCodingGuidelines(methodology: string): Record<string, string[]> {
